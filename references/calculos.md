...
O Gordon usa `CF_N × (1+g)` como primeiro fluxo terminal e, portanto, mantém o reinvestimento do último ano projetado. Ele só coincide com o terminal por RONIC quando `CF_N × (1+g) = NOPAT_{N+1} × (1 − g/RONIC)`. Informar `terminal_nopat` (NOPAT de N+1) e `ronic` junto com `terminal_growth` faz o script verificar essa igualdade, com tolerância relativa `terminal_tolerance` (padrão `0.0001`), e devolver `terminal_check`; a divergência é rejeitada com os reinvestimentos implícito e exigido. Não confundir taxa spot por prazo com taxa anual encadeada.
